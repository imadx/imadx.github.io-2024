	...
	const workplacesContent = worplaces.data
		...
		.map((workplace) => {
			return (
				<li
					key={workplace.title}
					className="flex place-items-center flex-col gap-1 justify-start w-[160px] text-center"
				>
					<a href={getSanitizedUrl(workplace.organizationUrl)}>
						<Image
							className={clsx(
								"size-[160px] flex place-items-center justify-center rounded-xl p-2",
								"hover:bg-shark-600/5 hover:grayscale-0 hover:brightness-100 hover:contrast-100 hover:shadow-xl",
								"dark:grayscale dark:brightness-150 dark:contrast-160",
								"bg-shark-950  hover:bg-shark-900",
